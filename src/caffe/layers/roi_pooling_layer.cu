...
__global__ void ROIPoolForward(const int nthreads, const Dtype* bottom_data,
    ...
    const Dtype* bottom_rois, Dtype* top_data, int* argmax_data, const int shape) {
...
void ROIPoolingLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  ROIPoolForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      ...
      pooled_height_, pooled_width_, bottom_rois, top_data, argmax_data, shape);
...
//   LOG(INFO) << "Backward_gpu @ ROIPoolingLayer:" << propagate_down.size() << "\n";
//   for (int i = 0; i < (int)propagate_down.size(); ++i)
//     {
//       LOG(INFO) << "propagate_down " << i << " " << propagate_down[i] << "\n";
//   }
// #endif
  ...
  ROIPoolBackward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      ...
      height_, width_, pooled_height_, pooled_width_, bottom_diff, bottom_rois, shape);
  CUDA_POST_KERNEL_CHECK;
}
